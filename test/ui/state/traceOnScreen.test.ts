...
describe('what replaces the resting trace', () => {
  it('a new run shows its own candidate A’s trace; a run with no candidates leaves none', async () => {
    const { state, b } = await generated();
    const promoted = reduce(state, { type: 'PROMOTE_CANDIDATE', payload: { candidateId: 'cand-b' } });
    ...
    expect(onScreen(rerun)).toEqual(traceOf(next, 'A'));
    expect(onScreen(reduce(rerun, inspect(null)))).toEqual(traceOf(next, 'A'));
