...
export function RudimentEventStepper({
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-gray-800/30 border border-gray-700">
      {/* Complexity badge */}
      <span className={`px-2 py-0.5 text-xs rounded border ${complexityColorClass}`}>
      ...
      <div className="flex items-center gap-1">
        <button
          className="px-2 py-0.5 text-xs rounded bg-gray-700 text-gray-300 hover:bg-gray-600 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          onClick={goPrev}
      ...
      {activeEvent && (
        <>
          ...
          <span className="text-xs text-gray-300 font-mono">
            ...
            {' | '}
            <span className="text-gray-200 font-semibold">{laneNameMap.get(activeEvent.laneId) ?? 'Unknown'}</span>
            {' | '}
            ...
            {' '}
            <span className="text-gray-500">[{activeEvent.pad.row},{activeEvent.pad.col}]</span>
          </span>
        </>
      )}
      ...
      {activeEventIndex !== null && (
        <button
          className="px-2 py-0.5 text-xs rounded text-gray-500 hover:text-gray-300 hover:bg-gray-700 transition-colors"
          onClick={clear}
      ...
      {/* Keyboard hint */}
      <span className="text-[10px] text-gray-600">← → to step</span>
    </div>
  );
}
