/**
 * Temporal Scenario Panel (left panel).
 *
 * Dropdown to select a temporal scenario, description text,
 * constraint tags, and a reset button.
 */
...
export function TemporalScenarioPanel({
  scenarios,
  selectedId,
  onSelect,
  onReset,
    ...
    <div className="flex flex-col gap-4">
          ...
          <span
            className={`text-[10px] px-2 py-0.5 rounded-full border ${
              selected.expectedInitialStatus === 'violation'
                ...
                : selected.expectedInitialStatus === 'degraded'
      ...
      {selected && (
        <div className="text-[10px] text-gray-500">
          {selected.moments.length} moments, {selected.moments.length - 1} transitions
        </div>
      )}
      ...
      {/* Notes */}
      {selected?.notesForWhyThisScenarioExists && (
        <div className="text-[10px] text-gray-500 bg-gray-800/50 border border-gray-700/50 rounded px-2 py-1.5 leading-relaxed">
          {selected.notesForWhyThisScenarioExists}
        </div>
      )}
