...
| P1a-5a | pass (deep: pending) | C3 greedy, beam and annealing Quick cases PWC (lock at [7,0] held, `placementLocks` non-empty in every candidate); the TEST MIDI 1 gate's lock cases for all three methods in `test:run`; locksHonoured.test.ts. The nightly deep-annealing lock case lost its `it.fails` in S1a.3 and has not run since: pending on [run 36072965133](https://github.com/TGALLOWAY1/PushFlow-Modified/actions/runs/36072965133). |
