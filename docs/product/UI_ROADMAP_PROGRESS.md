...
| lastOpenedAt | session | yes (schema 4) | Set when the editor opens the project, without touching updatedAt (S2.3) |
...
**Deliverables**
- [x] Library honesty and entry (T52, T51 slices, T53) · *PR / verified by:* commit bee6153. **Real data.** `persistence/projectIndex.ts` lists each stored project with its bars (to the last note's end at the project tempo), events (moments, through groupIntoMoments, decision Q7; the old count was notes: "48 events" for TEST MIDI 1's 32), notes, Sounds, BPM, created date and **last opened** date. Cards and the hero read "8 bars · 32 events · 7 Sounds · 120 BPM" and "Created Sep 20 · Opened 2h ago" (`Homepage/projectFacts.ts`), plus "2 variants". **Last opened** is a new stored field (schema 4, `lastOpenedAt`, migration `last-opened-at` behind the S1a.2 backup; the migration starts it at the last save). The editor records it when it loads a project (`markProjectOpened`), without touching updatedAt; the Library orders projects by last opened or created and its hero is the project opened last ("Last opened", one "Continue" button; "Resume Session" and "Open Layout Editor" did the same thing). **Opening no longer re-saves.** Every editor mount ran SYNC_STREAMS_FROM_LANES, which rebuilt identical Sounds but bumped updatedAt (and threw the analysis away), so autosave re-saved every project just opened; an equal rebuild is now a no-op, and "← Library" no longer writes the same state twice (its save, then the unmount flush). saveNow already wrote nothing when nothing was pending (S1a.4). **Thumbnails** draw the layout the project opens on (Q1) with its badge (`thumbnailLayout`), so a draft-only project shows its pads. **Import MIDI** is the Library's primary action: a MIDI file starts a project named after it ("TEST MIDI 1") with its Sounds, through the same import plan as the editor (`state/midiImportPlan.ts`, now shared by `useLaneImport`), and opens it with nothing placed and bottomLeftNote 36; a PushFlow project file picked there is recognised by its content ("MThd" or JSON), and anything else gets a plain-language message. **Open demo project** (the welcome panel and the sidebar's "Open the demo") does the same with `public/demo/TEST MIDI 1.mid`, named "Demo · TEST MIDI 1". **One "⋯" menu** on the hero and every card (`Homepage/ProjectMenu.tsx`): Rename (in place), Duplicate ("<name> (copy)", numbered, never opened, with its Composer pattern), Export project file, Download backup (only when one exists) and Delete, which acts at once with a 10 s Undo toast that puts the stored record and its backups back exactly (`deleteProjectWithUndo`, `restoreDeletedProject`); the native confirm and the hover-only × are gone. Export is also in the editor's new title menu (Rename project, Export project file). Words: "New project", "Your projects", "Projects", "Import MIDI", no "Performance(s)" or "Session" for the container; the icon-font glyphs (which rendered as the words "search", "add", "arrow_forward" whenever the remote font was blocked) are lucide icons; Library text is at least 11 px.
- [x] Variants worth keeping (T29) · *PR / verified by:* commit bee6153. "Save variant" opens a name field (`workspace/SaveVariantPopover.tsx`) pre-filled with the layout's base name and the time, "Default – 25 Sep 14:02" (`state/variantNames.ts`; role words such as "(suggested)" are dropped), numbered " (2)" when taken; SAVE_AS_VARIANT itself numbers a taken name, so no two variants share one. A toast confirms "Saved variant "…"" and the Layouts tab opens and scrolls to the new card. RENAME_LAYOUT gains a `variant` target (numbered when taken, ignored when empty); each variant card has a pencil (and double-click) to rename in place. Each card shows "Scoring..." and then "Score 79% · 0 hard · 0 unplayable" from the per-layout analysis cache (`useLayoutAnalysis`, S1b.3). Recovered drafts keep their own group. "Load Draft" reads "Load draft".
- [x] Guidance and copy (T44, T56) · *PR / verified by:* commit bee6153. One staged empty state at the grid (`workspace/GridEmptyState.tsx`): with no Sounds, a card in the grid's centre offers **Import MIDI** and **Build a pattern in Composer** (switches the drawer tab) and takes .mid files dropped on it (other files are refused with a toast); with Sounds but nothing placed, the state bar above the grid reads "Drag your 7 Sounds onto pads, or [Suggest a starting layout]" and stays off the pads, so Sounds and presets can still be dropped anywhere. The Analysis panel's duplicate Suggest button is gone (its note now points at the grid), the timeline's empty state says what it will show instead of offering a second import button, and "+ MIDI" stays the secondary import once Sounds exist. "Assign sounds to pads, then Generate to analyze" and "Click Generate to create candidate layouts" now read "Analysis updates automatically as you place Sounds · Generate proposes alternatives" and "Generate proposes alternative layouts to preview, compare and keep"; Learn More's App Flow follows (Import, Place, Analyze, Generate, Compare, Keep, Promote). Sentence case: "Save variant", "Calculate cost", "Event difficulty chart", "Load draft", "New project".
- [x] Gear cleanup (T39 slice) · *PR / verified by:* commit bee6153. "Organize by 4x4 Banks" (read by nothing) and "Duplicate Layout" (a hidden second Save Variant) are gone, with `LayoutDisplaySettings`. View settings are remembered per viewer (`pushflow:view-settings` in localStorage, wrapped in try/catch; odd values fall back to the defaults). "Show Finger Assignment" is on by default and still shows solver fingering.

**Exit criteria**
- [x] **P2-5a** Library: cards show BPM, Sound count, bar length, event count, created date and last-opened date from real data. · *PR / verified by:* `library-trust.spec.ts` "cards show real data …" at 1366 and 1600 (TEST MIDI 1: "8 bars · 32 events · 7 Sounds · 120 BPM", "Created Sep 25 · Opened just now"; the empty hero "No Sounds yet · 120 BPM"); `libraryFacts.test.ts` (bars, events vs notes, dates, never opened, the order and the hero pick). Against the S2.2b Library the card read "48 EVENTS" with no bars, created or opened date.
- [x] **P2-5b** Library: a draft-only project shows its pads. · *PR / verified by:* the same spec: the draft-only card's thumbnail has 7 placed cells and the badge "Draft, not promoted" (the S2.2b card drew an empty grid); `libraryFacts.test.ts` "a draft-only project shows its draft …" and the Active case.
- [x] **P2-5c** Library: importing a .mid from the Library creates "TEST MIDI 1" with 0 pads placed and bottomLeftNote 36. The demo project opens unplaced. · *PR / verified by:* `library-trust.spec.ts` "Import MIDI creates "TEST MIDI 1" with nothing placed, and the demo opens unplaced" at 1366 and 1600 (name, 7 Sounds, no pads in Active or a draft, bottomLeftNote 36; the demo "Demo · TEST MIDI 1" likewise); `libraryActions.test.ts` (the same through `projectFromMidiFiles`, and file-kind detection by content). Before S2.3 the Library accepted only .json and a .mid gave "Unexpected token M … is not valid JSON".
- [x] **P2-6** Two variants saved in the same minute get distinct names. A renamed variant keeps its name after a reload. Each variant card shows "Scoring..." and then a score. · *PR / verified by:* `library-trust.spec.ts` "distinct names in the same minute; a rename survives a reload; each card is scored" at 1366 and 1600 (two saves through the name field, the second "… (2)"; rename to "Wide hands"; saveAndReload; both cards read "Score N% · N hard · N unplayable"); `variantCard.test.tsx` ("Scoring..." while the analysis is pending, then "Score 82% · 1 hard · 1 unplayable"; rename in place, Escape keeps the old name); `libraryFacts.test.ts` (names, same-minute numbering, the reducer's uniqueness and the variant rename target).
- [x] **P2-7** Deleting a project and pressing Undo within 10 s restores it with the same id, layouts and variants (T53). · *PR / verified by:* `library-trust.spec.ts` "Delete, then Undo within 10 s, restores the project exactly" at 1366 and 1600 (the stored record before Delete equals the one after Undo, and none exists in between); `libraryActions.test.ts` (record and backups restored exactly).

**Session checks**
- [x] Full suite before push. · *PR / verified by:* `npm run typecheck`; `npm run test:run` (97 files, 1116 passed, 4 `runIf` skips); Playwright at 1366, 1600 and (C1) 1920: 161 passed, 2 failed, the Library screenshot comparisons: the Library is redesigned here, so its baselines are regenerated in CI (`update-snapshots`). The axe check of the Library passes on the new page.
- [x] Stored data: schema 4 through the migration runner. · *PR / verified by:* `migrations.test.ts` "last-opened-at (schema 3 → 4)" (starts at the last save, keeps an existing value, changes nothing else, running it again changes nothing); `projectStorageMigration.test.ts` (backup first, then v4, once); `projectDocument.test.ts` (a project saved by main round-trips with only the migrations' fields added).
...
**Deliverables**
- [x] Shortcut registry and input table (T61 slice) · *PR / verified by:* commit 1e21d97. **One table.** `src/ui/input/inputTable.ts`: `INPUT_TABLE`, 20 rows in five groups (Placing Sounds, Pads, Playback and events, Editing, Help), each with what is pressed or clicked, the mode it applies in, what it does and, for keys, a matcher. Rows for later phases carry `from` (Alt-click to audition, P4; Enter on a focused pad, P8): listed so the meaning is reserved, not bound, and left out of the sheet. **One listener.** `src/ui/input/inputRegistry.ts`: components bind handlers to rows (`useInputHandler`), and one document listener matches each key against the table and calls the highest-priority, latest binding (a handler that returns false passes the key on). It leaves alone keys in text fields, selects, contenteditable, menus, listboxes and dialogs (by role, or any open Dialog or Popover), except rows marked `anywhere` (Save), and keys a widget already handled, so a focused widget's own keys win. The window key listeners in EventsPanel (↑/↓/j/k), VoicePalette (Mod+G) and PerformanceWorkspace (M during a preset drag, which browsers never delivered: the S1b.2 follow-up) are gone; `useKeyboardShortcuts` binds the editor's rows. **Keys.** Space plays and stops anywhere but a text field or select, with preventDefault on keydown and keyup, so a focused button isn't pressed; while its tab is shown the Composer binds Space at a higher priority and plays its pattern. ←/→ select the previous or next event while stopped and stop at the first and last (they wrapped). ↑/↓/j/k do the same while focus is in the Events list. Escape steps back one layer: an open overlay closes itself (the Overlay primitive, first), then the armed Sound, then the pad selection, then the event. Delete/Backspace take the selected pad's Sound off with the Undo toast (a locked one says why); with no pad selected they still do nothing (T28). "?" opens the sheet. **Pad clicks** (`padClickMeaning`): with a Sound armed, place it (below); with an event selected, select the pad and keep the event; otherwise select the pad and its Sound, where a click selected that Sound's first hit and lost the event (T28); an empty pad clears the selection; Alt-click is reserved and does nothing yet. The selected pad has a neutral outline and stays undimmed while an event is shown. `armedStreamId` and `selectedPadKey` are session state (never saved, never in undo; Undo clears a pad selection whose layout changed). **The sheet.** `shared/ShortcutSheet.tsx`: "Keyboard and mouse", a Dialog generated from the table's bound rows by group, with ⌘, ⇧ and ⌥ on a Mac and Ctrl, Shift and Alt elsewhere. It opens on "?", from the gear menu ("Keyboard and mouse ?") and as Learn More's new "Keyboard & mouse" tab (the same sections). The gear popover and the Sounds panel's colour popovers are dialogs to the table (role dialog), so Space toggles their focused option, and the gear closes on Escape. Tests: `test/ui/input/inputTable.test.tsx`, `test/e2e/input-table.spec.ts`.
- [x] Click-to-place (T62 slice) · *PR / verified by:* commit 1e21d97. A plain click on a Sound row arms it (`ARM_SOUND`; the armed Sound is also the selected one): the row gets an accent ring and a crosshair, an empty pad under the pointer previews it (a dashed outline in its colour, the copy cursor, "click to place …" in the tooltip), and the state bar reads "A · click an empty pad to place it [× Stop placing]" (`ArmedSoundHint`). A click on an empty pad places it with one `ASSIGN_VOICE_TO_PAD`, so one undo step ("Place Sound"), into the Working/Test Layout as a drop does: an explicit, named action (invariant 7). **Next unplaced:** a placement arms the next Sound no pad holds, in the Sounds list's order, so one Sound click and seven pad clicks place TEST MIDI 1. Arming is idempotent (clicking the armed row keeps it armed), so it never undoes the auto-advance. An armed Sound already on the grid moves to the pad clicked, and placing ends. An occupied pad says "Pad taken · drag to swap" and changes nothing; the armed Sound's own pad ends placing; a locked Sound says "A is locked to Row 1 · Col 1 · Unlock it to move it". Mod- and Shift-clicks still select several Sounds for Mod+G, without arming. Escape or "Stop placing" ends it. The "nothing placed" line reads "Place your 7 Sounds: click one, then a pad (or drag it), or [Suggest a starting layout]", and the Analysis note, the empty-grid card and Learn More's App Flow say the same.
- [x] Composer geometry (T69 slice) · *PR / verified by:* commit 1e21d97. **One geometry.** `LoopGridCanvas`: one whole-pixel `cellWidth` (`composerCellWidth`: the container split evenly across the steps, never under 16 px) places the cells, the bar and beat headers, the grid lines and the playhead. Cells and headers are `flex-shrink-0` and the grid is exactly `cellWidth × steps` wide, so nothing shrinks away from the lines (at 1600 with 16 bars, cells were 10.5 px and bar headers fell every 84 px against lines every 96 px). A grid that doesn't fit scrolls sideways (`composer-grid-scroller`); the lane column sits outside the scroller, so it stays in place, sticky in effect. **One toolbar row.** `role="toolbar"`, `flex-nowrap` (a drawer too narrow for it scrolls it instead of wrapping), with fixed-width controls (`COMPOSER_TOOLBAR_WIDTHS`); Save Preset keeps its border while disabled, so enabling it changes no width. The note count moved to the lane column's header ("Lanes · 5 notes · live sync": notes, per Q7), and the title block went (the tab names the Composer; its sentence is the count's tooltip). At 1366 the toolbar took three rows. **The drawer holds still while composing.** A first note on a new lane adds a Sound, and the drawer, fitted to the timeline's Sound count since S2.1, grew one lane each time, moving the whole Composer up 32 px under the pointer. With the Composer tab shown, the drawer now takes its 40% default (or the viewer's height), which doesn't follow the Sound count. The Bars and Grid buttons have aria-pressed. Screenshots: `docs/screenshots/S2.4/`.

**Exit criteria**
- [x] **P2-10** Every input-table row has a passing registry test. Space toggles playback while a button has focus. ArrowDown on a focused select changes the select, not the moment. · *PR / verified by:* `inputTable.test.tsx`: one test for each of the 20 rows (`ROW_TESTS` is a Record over the row ids, so a row without a test doesn't compile, and a run-time check compares the lists), plus the registry's skip rules, priorities and overlay handling, the pad-click table, and that the editor binds every bound key row and nothing reserved (29 tests). `input-table.spec.ts` at 1366 and 1600: "Space plays and stops while a button has focus, without pressing the button" (Loop focused: play, stop, Loop unchanged, focus kept), "Space in a text field types a space and doesn't play", "ArrowDown on a focused select changes the select, not the event" (Speed 1× → 1.25×; the selected event stays, for → too) and ""?" opens the sheet generated from the table; Escape closes it" (also from the gear menu).
- [x] **P2-11** All 7 Sounds can be placed with click-to-place alone (Playwright). · *PR / verified by:* `input-table.spec.ts` at 1366 and 1600: "all 7 Sounds placed by clicking a Sound and then a pad, one undo step each" (7 pads, undo depth +7, nothing left armed), "with the auto-advance, one Sound click and seven pad clicks place them all" (with "Pad taken · drag to swap" on an occupied pad; one Undo takes back one placement) and "Escape stops placing; a click then selects the pad, and Delete takes its Sound off with Undo". Before S2.4 a click on an empty pad placed nothing.
- [x] **P2-13** The Composer's bar-8 line aligns with its cells within 1 px at 1366 and 1600, and enabling Save Preset doesn't move the grid. · *PR / verified by:* `input-table.spec.ts` at 1366 and 1600: "with 8 bars" and "with 16 bars, bar 8's header, line and first cell line up within 1 px" (cells at least 16 px and edge to edge; the bar header 8 cells wide), "the first note enables Save Preset without moving the grid, and the toolbar is one row" (measured after the note's Sound lands in the project) and "a first note on each new lane adds a Sound without moving the Composer" (three lanes); `gridSizing.test.ts` (the Composer's drawer height). Before S2.4, at 1600 with 16 bars bar 8's line sat 84 px right of its header, the toolbar wrapped when Save Preset enabled, and each new lane's first note moved the Composer 32 px.

**Session checks**
- [x] Full suite before push. · *PR / verified by:* `npm run typecheck`; `npm run test:run` (98 files, 1146 passed, 4 `runIf` skips); Playwright at 1366, 1600 and (C1) 1920: 183 passed, 2 failed, the Library screenshot comparisons (about 1% of pixels: the cloud-container mismatch of the S1a.1 follow-up). S2.4 doesn't change the Library; CI's baselines were regenerated for S2.3's redesign (9c58e04).
- [x] C1 bounds the pad menu's width (P1b audit follow-up). · *PR / verified by:* `c1-pad-menu.spec.ts` "opens at the cursor, or clamped fully inside the viewport, for all 64 pads" now also requires the menu to be under 320 px wide, at 1366, 1600 and 1920 (9 passed).
...
**Deliverables**
- [x] An accessible primitive kit ships first (T63 part) · *PR / verified by:* PR #109, commits 15bc1ce, 79eb296 (and 4c2214f's notes). Five primitives in `src/ui/components/shared/`, each requiring an accessible name at the type level and rendering its role and state. **Tabs** (`Tabs.tsx`): a tablist named by `label` or `labelledBy`; `role=tab` buttons with aria-selected and aria-controls, paired with `role=tabpanel` (aria-labelledby); unselected panels stay in the DOM, hidden, so every aria-controls resolves (`keepMounted` keeps their content mounted); roving tabindex (only the selected tab, or the first when none is, is a Tab stop); ←/→ wrap and Home/End move and select, with preventDefault; per-tab `testId`/`panelTestId` so existing selectors can move onto it; `listEnd` puts extra controls in the tab row outside the tablist. **ToggleButton**: a `<button aria-pressed>` with a required `label` (`hideLabel` makes it the aria-label and tooltip), filled when on and outlined when off, at least 24 px. **Checkbox**: a native checkbox drawn with tokens, with a visible `label` or a required aria-label (never both), `indeterminate`, a ≥24 px label target and `disabledReason` (the DisabledReason pattern). **IconButton**: a required `label` becomes the aria-label and the default tooltip; the icon is aria-hidden; 24, 28 or 32 px; `disabledReason`. **Card**: a plain container (its types refuse onClick), or, with `action`, a real `<button>` or `<a>` named by `action.label` and stretched under the card's content (`.pf-card-action`), so the card's own controls stay separate valid controls above it rather than nested in a button; `action.current` sets aria-current. Every focusable part uses `.focus-ring`, now drawn in `--border-focus` (7:1 or more on every surface; the accent fell to 2.8:1 on hovered ones). **Keys** (`inputRegistry.ts` `widgetOwnsKey`, by role): Space ticks a focused checkbox (`role=checkbox`/`switch` too) and ←/→/Home/End move between tabs inside a tablist, and only those keys (←/→ on a focused checkbox still step events; Space on a button or a tab still plays, the S2.4 rule); a Space keyup belongs to the latest keydown. The "?" sheet's Space and ←/→ rows say so. No existing panel is migrated (P7's sweep); later P3 surfaces use the kit. Tests: `test/ui/components/primitives.test.tsx`, `test/ui/input/inputTable.test.tsx` (the `space` and `step-events` rows with a focused Checkbox and Tabs; "leaves a focused widget its own keys by role, and only those (T63)").
- [x] One yardstick (T21 slice) · *PR / verified by:* PR #109, commits b07ac2f, 9141a1a, 6623a72. **Playability** (`engine/evaluation/playability.ts`, `scoreLayoutPlayability`, exported from `@/engine`): a layout is scored on the fingering of its own analysis plan (each pad's finger plus the moments where the plan re-fingers a pad, as the beam solver's `canonicalBreakdown`) by the canonical evaluator (`evaluatePerformance` over `buildPerformanceMoments`, with the cost toggles and Calculate Cost's evaluation config). Playability = `computePlanScore` of the canonical hard events (fallback grips), the canonical unplayable events (a note with no pad or finger, one finger on two pads, a move too fast for a hand; unplaced notes included until S3.3) and the canonical cost per event, so it depends on the layout alone, never on which optimizer proposed it. It returns a `LayoutScore` (`canonical-v1`: playability, events, hardEvents, unplayableEvents, costPerEvent, the five factors and their total, the canonical verdict); with all toggles on, its factor total equals the beam plan's own to 1e-9. **One cache entry per key**: `{ analysis, score }` under `canonical-v1` (the `beam-fast` entries are gone), capacity 16 → 64, keyed and solved as the layout with its finger preferences re-derived from voiceConstraints (`withDerivedLayoutState`, what applying a candidate produces), so a candidate and the draft it becomes share one entry: Greedy keeps a moved Sound's preference on the pad it left (`{"2,3":"L2"}` on the candidate, `{"3,3":"L2"}` as the draft), so the raw hashes differ. **Worker**: the solve and the score run in a module worker (`scoring.worker.ts` behind `scoringClient.ts`: request ids; in-process where there is no Worker, as in vitest; a failed worker's owed and later requests run in-process), `worker.format: 'es'`; the build emits `assets/scoring.worker-*.js` under `/PushFlow-Modified/`. **The draft on the same path**: auto-analysis takes its plan from the same cache and worker (debounce, abort and both isProcessing resets kept). `useLayoutAnalysis(layout)` returns the plan (bound to the layout) and the score, and keeps a row's result after the cache evicts it, so rows never go back to "Scoring…" and re-solve (the old hook's effect thrashed: 104 solves for 72 rows over a 64-entry cache). **Every Score on screen reads it**: candidate cards, the Active row (new), variant rows (whose hard and unplayable counts now come from the same LayoutScore), Compare's Score per side, and the Score tile of both Analysis panels for the layout the grid shows; "Score N%" and its colours stay (S6.1 changes the presentation), with the tooltip "Playability · canonical evaluator · higher = easier", "Scoring…" until ready and "Couldn't score" on error. Learn More's Cost Factors tab explains the Score from `PLAN_SCORE_WEIGHTS` and FACTOR_META. On TEST MIDI 1 (Greedy, all strategies), "Exploratory Variant (seed 1)" read 90.6 on its row and 68.0 as the draft or a variant; it now reads 88 on both. Tests: `test/engine/evaluation/playability.test.ts`, `test/ui/analysis/analysisCache.test.ts`, `scoringClient.test.ts`, `layoutAnalysisHook.test.tsx` (no thrash: with the old effect it fails "expected … 72 times, but got 104 times"), `test/ui/components/oneYardstick.test.tsx`, `test/e2e/one-yardstick.spec.ts`, the Learn More sync test. Screenshots: `docs/screenshots/S3.1/yardstick-*` (before, with Natural Pose: row and tile 85% while a variant of the same pads read 91%, and Compare "79.4 / 85.3"; after: 91% everywhere, Compare "79% / 91%").

**Exit criteria**
- No roadmap criterion is owned here. P3-4, owned by S3.3, re-checks the second session check below through "Use as my draft".

**Session checks**
- [x] getAnalysisForLayout returns the same Playability for a layout on the working-layout path and on the candidate path. · *PR / verified by:* PR #109 (b07ac2f, 9141a1a). `oneYardstick.test.tsx` "check 1": a real Greedy (Natural Pose) candidate on TEST MIDI 1 that moves a Sound with a finger preference (so the raw layout hashes differ): the candidate path (`analyseLayoutCached`) and the working-layout path (auto-analysis after APPLY_GENERATION_TO_LAYOUT in a real ProjectProvider) share one key, one LayoutScore and one solve, the draft's analysisResult is that plan, and each path solved afresh with the cache cleared gives an equal score. `analysisCache.test.ts` "re-derives finger preferences…" covers the key.
- [x] A greedy candidate applied as the draft through today's Preview scores the same before and after, and the variant cards and Compare read that value. · *PR / verified by:* PR #109 (b07ac2f). `oneYardstick.test.tsx` "check 2" (LayoutOptionsPanel, ActiveLayoutSummary and useAutoAnalysis in a real ProjectProvider): candidate #1's row reads "Score: N%"; after its Preview button the Analysis Score tile reads N% (while previewed and as the draft); a variant saved from the draft reads "Score N% · h hard · u unplayable" from the same LayoutScore; Compare (Active against the candidate) reads N% for the candidate and the Active row's number for Active. `one-yardstick.spec.ts` at 1366 and 1600: row #1's score before Preview equals the Analysis tile after, and `__pf.scoring()` reports worker > 0, inline 0. `c7-compare.spec.ts` compares Playability with Playability.
- [x] Full suite before push. · *PR / verified by:* on the merge of both halves (adc804c): `npm run typecheck`; `npm run test:run` 105 files, 1210 passed, 4 `runIf` skips; `npm run build` + `npm run check:no-test-hook` ("OK: no window.__pf in dist/"; the worker is `assets/scoring.worker-*.js`). Playwright (PW_PORT per checkout, chromium-1366 and -1600) on each half before the merge: `one-yardstick` 2/2, `c7-compare` 10/10, `clean-names` 4/4, and every other spec each half could affect (`library-trust`, `c2`, `c3`, `c4`, `c5`, `c6`, `c8`, `c9`, `readable-names`, `overlays`, `hook`, `readability`, `measured-grid`, `input-table`, `composer-data`, `distinct-sounds`) passed, apart from C2's S3.2 and C9's S8.2 expected-fail cases and one readable-names greedy Generate that timed out at a load average of about 15 and passed 2/2 when re-run alone. TEST MIDI 1: no optimizer code changed; `testMidi1Integration.test.ts` passed in the suite.
- [x] The new primitives have role/state tests. · *PR / verified by:* PR #109 (15bc1ce, 79eb296). `primitives.test.tsx` (happy-dom, 20 tests): roles, names and states (tablist/tab/tabpanel pairing, aria-selected, aria-pressed, checked and indeterminate, aria-describedby reasons, aria-current); keys (arrows wrap, Home/End, modified keys ignored; Space on a Checkbox ticks it and doesn't play, through a simulated keydown, keyup and click; Space on a ToggleButton plays and leaves it unpressed); focus (arrows move it; roving tabindex, also with nothing selected); a Card's own controls are never nested in its button; `.focus-ring` on every focusable part. "requires its label props" compiles its own file with the project's compiler options, so each of its 7 `@ts-expect-error` lines must be a real error (making one valid fails the test with "Unused '@ts-expect-error' directive").
...
**Deliverables**
- [x] inspectedLayout selector · *PR / verified by:* PR #109, commit bffbed6. Two session fields, never in undo and never saved: `inspectedLayout` (`{kind: active | working | candidate | variant | recovered, id}`) and `inspectedAnalysis` (a read-only layout's own plan from the per-layout cache, mirrored by `useInspectedAnalysis` so the pure selectors the timeline, the Events list and SELECT_EVENT's playhead use can read it, and ignored unless `checkPlanFreshness` matches the layout shown). One resolver, `resolveInspectedLayout`, feeds `getInspectedLayout`, `isInspectedReadOnly`, `getInspectedCandidate`, `getDisplayedCandidate`/`getDisplayedExecutionPlan` (now the plan on screen: a read-only layout's own plan from the cache, never a candidate's optimizer plan), `getActiveTrace`, `isReplayingTrace` and `isShownLayoutReadOnly`; `getDisplayedLayout` still names the layout edits go to. A draft identical to Active shows as Active (the S1a.4 follow-up), and Active is read-only only over a differing draft. The grid (through `layoutOverride`), the timeline, Events, both Analysis panels, the Sounds panel, Calculate Cost, Compare, the trace panel and the e2e hook read it; `selectedCandidateId`, SELECT_CANDIDATE, `getSelectedCandidate` and `getDisplayedLayoutRole` are gone. **Looking never writes**: INSPECT_LAYOUT touches no document field, and the reducer refuses the nine layout-edit actions (`LAYOUT_EDIT_ACTIONS`: pad assigns, merges, removals, swaps, pad finger preferences, locks, starting a draft, Suggest) while a read-only layout or a trace replay is shown, whatever dispatched them; the UI refuses every edit path with the toast "Use as my draft to edit" (Sounds-panel and preset drops, pad drag and swap, click-to-place, the pad menu, the pad's ×, Delete/Backspace, the selected note's finger controls, which are disabled with their reason). Undo and Redo still work; `restoreDocument` drops an inspection that no longer resolves. Tests: `test/ui/state/inspectedLayout.test.ts` (17: the resolver; "inspecting every layout 20 times leaves the document, and so the draft hash, unchanged"; the guard over 5 read-only views × 9 actions; named role actions still run; a Sound's finger preference applies; deleting the inspected layout; Undo/Redo; the plan on screen; the trace follows Inspect), `projectDocument.test.ts`, `projectSerializer.test.ts`.
- [x] After a run, candidate A is auto-inspected read-only · *PR / verified by:* PR #109 (bffbed6). SET_CANDIDATES shows candidate A read-only and writes nothing, even on an empty grid (Q4); a run with no candidates while one is shown returns to the draft. It lives in the reducer, so every path that installs a run's candidates inspects A (`generateFull` is unchanged). A one-time note in the bar ("Read-only · Use as my draft to edit it; your draft stays as it is", "Got it"; per viewer in localStorage `pushflow:coach:use-as-draft`, in try/catch) shows on the first candidate until dismissed or a candidate action is taken. Tests: `inspectedLayout.test.ts` "after Generate, candidate A is shown read-only, even on an empty grid (Q4), and nothing is written"; `generateOnlyProposes.test.tsx` "P1a-2a: after Generate the draft hash is unchanged and candidate A is shown read-only" and "P1a-4…"; `inspect.spec.ts` (below); `layoutStateBar.test.tsx` "the note on the first candidate shows once per viewer…" (also with storage blocked).
- [x] Explicit role actions · *PR / verified by:* PR #109 (bffbed6). Only the role on screen gets actions. **Candidate**: Use as my draft, Promote (PROMOTE_CANDIDATE), Keep as variant (SAVE_AS_VARIANT source 'candidate', named after its strategy through `strategyLabels`, provenance `candidate:<strategy>`), Back to my draft. **Variant**: Use as my draft ("Edit as draft" on its row, was "Load draft"), Promote (PROMOTE_VARIANT), Back. **Recovered draft**: Use as my draft, Back. **Draft**: Promote, Save variant (SaveVariantPopover) and Discard (its Undo toast, "· Finger preferences kept"), moved out of the top bar into the state bar. **Active over a draft** (a click on the Active row's Inspect): "Viewing Active · your draft is kept" and Back to my draft. Use as my draft asks "Save my draft as a variant first" or "Replace (undoable)" when a differing draft exists and otherwise applies at once; either way one undo step (`transact`), and a replaced draft still goes to Recovered drafts (see Deviations). Preview is now Inspect (candidate, Active, variant and recovered rows) and writes nothing; a card-body click does nothing; the P1a Preview auto-keep is retired. Tests: `LayoutOptionsPanel.test.tsx` (8, including "a click on a candidate card body neither inspects nor writes", "Inspect shows the candidate read-only and writes nothing…", "Edit as draft asks before replacing…one Undo brings it back" and "…Save my draft as a variant first: one undo step…"); `layoutStateBar.test.tsx` (candidate, variant, Promote, Use as my draft and Discard cases); `inspectedLayout.test.ts` "named role actions still run…"; the C2 recoverability cases; `c7`, `overlays` ("Promoted Candidate A to Active Layout"), `one-yardstick`.
- [x] Layout-state bar (T03) · *PR / verified by:* PR #109 (bffbed6). `workspace/LayoutStateBar.tsx` fills the fixed, unscaled 36 px state-bar slot above the grid (see Deviations for the height): a RoleChip with its own colour and icon from the new `--role-active`, `--role-working`, `--role-candidate` (violet `#a78bfa`) and `--role-variant` tokens (index.css, tailwind.config.js); the name (`layoutLabel`, or the strategy label for a candidate, whose chip reads "Candidate A"); "N pads vs Active" or "Same pads as Active" (plus "· N vs your draft" on a read-only layout, via layoutDiff); freshness, "Up to date", or "Updating…" once the analysis has been pending 1.5 s (it keeps its last word for the same layout meanwhile, so a quick re-analysis never flickers); and that role's actions. The S2.4 placing hints stay in the bar. The top bar's "Analysis outdated" badge is gone. The transition preview moved from the slot into the selected-event card, and its arrows toggle into View settings ("Show Transition Arrows"). **One SubjectChip** (`shared/SubjectChip.tsx`) heads the Layout Summary, Cost Analysis, Events, the selected-event card, the difficulty chart and its dialog, both Compare sides ("Candidate A · name") and the timeline header ("Timeline shows: …"). Candidates are lettered A, B, … by `candidateLetter` (S3.3 makes the letters stable). Timeline pills and "Show Finger Assignment" use the plan on screen and still read hand and finger ("L2"); a note click still selects its whole event. The trace panel and its step-through follow the inspected candidate, and a replay is read-only ("Replaying trace step N of M · read-only", Exit replay). Learn More's App Flow tab gains "Which layout is on screen" (from `ROLE_META`), and the input table a 'read-only-edit' row. Tests: `layoutStateBar.test.tsx` (11), `LearnMoreModal.test.tsx` ("lists every role…", "says Generate shows candidate A read-only…"), `state-bar.spec.ts`, `measured-grid.spec.ts` (pads at least 32 px and the slot visible at 1366 and 1600), `readability.spec.ts`. Screenshots: `docs/screenshots/S3.2/inspect-before-*` and `inspect-after-*` (1366 and 1600).
- [x] Clean names (T32), with its layout-name migration · *PR / verified by:* PR #109, commits e80cd2b, 4c2214f (built alongside S3.1; see Deviations). `Layout.provenance` is `LayoutProvenance` (manual | suggested | `candidate:<strategy>` | `variant:<id>` | recovered | replaced-active; the old `'recovered'` and absent still read), set wherever a layout is made: a manual edit's draft is `manual`, Suggest `suggested`, a used candidate `candidate:<strategy>`, Load draft `variant:<id>`, a saved variant keeps its draft's (a kept candidate gets its strategy), a recovered draft `recovered`. No reducer writes "(draft)", "(suggested)" or "(replaced <date>)" into a name: every Promote keeps the clean base name, and the replaced Active is auto-saved by one helper (`withReplacedActiveSaved`) as "Default – 25 Sep 14:02" (numbered when taken, `replaced-active`). Labels are built from role and base name by `layoutLabel` (`src/ui/state/layoutLabels.ts`: "Draft of Default"; with `withRole`, "Active: Default", "Saved variant: …", "Recovered draft of …", "Candidate: …"), used by the Layout Summary, the Active card and Recovered drafts; Save variant's default name strips role words the same way. Migration `clean-layout-names` (schema 4 → 5, after the runner's backup) takes role words out of every stored layout name (Active, draft, variants, recovered drafts; anywhere in the name, repeated, any date format), sets provenance where the last role word allows ("(suggested)" → suggested; a variant's "(replaced …)" → replaced-active; existing provenance kept), re-dates replaced Actives from their savedAt, numbers a variant name it makes taken, and never leaves a name empty. Tests: `test/ui/state/layoutLabels.test.ts`, `test/ui/components/layoutNames.test.tsx`, `migrations.test.ts`, `projectStorageMigration.test.ts`, `projectDocument.test.ts` (the saved-by-main fixture's "Default (suggested)" → "Default", provenance suggested), `test/e2e/clean-names.spec.ts`, fixture `test/fixtures/projects/role-suffixes.json`. Screenshots: `docs/screenshots/S3.2/names-*` (before: the summary reads "Default (suggested) (draft)" and the Active card "Default (suggested)"; after: "Draft of Default" and "Default").

**Exit criteria**
- [x] **P3-1** Inspecting Active, every candidate and every variant 20 times in a row leaves the workingLayout hash unchanged. After Generate, the workingLayout hash is unchanged; the grid shows candidate A read-only under the violet bar, and "Back to my draft" shows the untouched draft. Drag, menu, click-to-place and Delete are each blocked while inspecting (one test per path). · *PR / verified by:* PR #109 (bffbed6). `test/e2e/inspect.spec.ts` at 1366 and 1600 (6/6 each): "inspecting Active, every candidate and every variant 20 times in a row leaves the draft hash unchanged" (and the undo history); "after Generate the draft is unchanged, candidate A is on the grid read-only under the violet bar" (chip colour #a78bfa) and "Back to my draft" shows the draft; and one test each for drag (pad to pad, and a Sounds-panel drop), the pad menu, click-to-place and Delete, each asserting the hint toast, an unchanged draft hash and grid, and no undo step. Component level: `readOnlyInspection.test.tsx` (the same paths plus a preset drop, the pad's × and the note's finger controls); reducer level: `inspectedLayout.test.ts`. With the UI refusals and the reducer guard removed, every path fails (drag: "expected false to be true", the dragover accepted; menu: "expected <div role=\"menu\" …> to be null"; click-to-place: "expected true to be false"; Delete: the "Removed …" toast appeared; preset: "expected 1 to be +0"; guard: "ASSIGN_VOICE_TO_PAD on Active over the draft").
- [x] **P3-2** The state bar shows the correct role and name for Active, Working/Test, Candidate and Variant at 1366 and 1600. The Analysis, Events and timeline headers name the same subject. While candidate B is inspected, timeline pill fingering equals B's plan (test). · *PR / verified by:* PR #109 (bffbed6). `test/e2e/state-bar.spec.ts` at 1366 and 1600 (2/2 each): "names Working/Test, Active, Saved variant and Candidate right…" reads the bar's role, chip and name for the draft ("Draft of Default"), Active ("Default"), a saved variant ("Wide hands"), Active over the draft ("Viewing Active · your draft is kept"), and candidates A and B (the name matches the row), and each time the Layout Summary, Cost Analysis, Events and timeline headers carry the same role, chip and name; "while candidate B is inspected, every timeline pill reads B's plan…": every pill's finger equals B's plan from the per-layout cache (`__pf.fingering({kind:'candidate', id})`), every pill reads `^[LR][1-5]$`, `fingering('shown')` equals B's plan, the grid's finger labels are B's, and a note click rings every pill at that time.
- [x] **P3-10a** After migration, no stored layout name contains "(draft)" or "(suggested)", and a re-run changes nothing. · *PR / verified by:* PR #109. `migrations.test.ts` "P3-10a: afterwards no stored layout name contains…" (9 fixture names, 7 with role words including "(draft) (suggested) (draft)", "(replaced 9/24/2026)", "(replaced 24.9.2026)", "(draft) variant" and a bare "(draft)"; none remain), "running it again changes nothing" and "changes nothing but layout names and provenance"; `projectStorageMigration.test.ts` (the backup `v4` is written before the `v5` project, equals the untouched record, and a second load writes nothing); `clean-names.spec.ts` at 1366 and 1600 (a schema-4 record in IndexedDB is backed up as `@v4`, stored at v5 with clean names, and a reload migrates nothing). Against the pre-S3.2 src the migration test fails with "expected [] to deeply equal [ 'clean-layout-names' ]", and a manual edit starts a draft named "Default (draft)".

**Session checks**
- [x] Flip the S3.2 C2 case. · *PR / verified by:* PR #109 (bffbed6). "inspecting a candidate never writes the Working/Test Layout" now clicks Inspect on every row and also asserts `__pf.inspected()`; its `test.fail` marker is removed. The recoverability cases follow the new actions (Inspect, a card-body click, Edit as draft → Replace, Use as my draft with save-first, candidate and variant Promote); `c2-draft-overwrite.spec.ts` 8/8 at 1366 and 1600. Before the flip, on a01090e with `PF_UNMARK=1`: "after inspecting candidate #1: Expected [the draft's pads] Received [candidate #1's pads]" (Preview wrote candidate #1 into workingLayout).
- [x] Full suite before push. · *PR / verified by:* on bffbed6 (fast-forwarded onto the phase branch, same tree): `npm run typecheck`; `npm run test:run` 108 files, 1252 passed, 4 `runIf` skips; `npm run build` (one new warning: the ProjectEditorPage chunk is 506.75 kB, over Vite's 500 kB warning; see Follow-ups) + `npm run check:no-test-hook`; Playwright at chromium-1366 and -1600: `inspect`, `state-bar`, `c1`–`c9` (c1 also at 1920), `overlays`, `input-table`, `measured-grid`, `readability`, `readable-names`, `library-trust`, `one-yardstick`, `clean-names`, `distinct-sounds`, `composer-data` and `hook` all passed, except C9's S8.2 expected-fail case, which still fails as documented. TEST MIDI 1: no optimizer code changed; `testMidi1Integration.test.ts` passed in the suite.
...
**Deliverables**
- [x] Single Promote (T13) · *PR / verified by:* PR #109, commit cd9779e. One hook, `hooks/useLayoutActions.tsx` (`LayoutActionsProvider` mounted once in ProjectEditorPage), serves the state bar, candidate rows, View all, variant rows and both Compare sides. Promote acts at once (the native confirm() and the timed "Confirm?" are gone) as one undo step with one toast, e.g. "Promoted Candidate F to Active Layout · Feasible · 2 Sounds not placed · your draft is in Recovered drafts" with Undo (withdrawn once Undo would revert another step). The plan the user reviewed, the layout's own plan from the per-layout cache, travels with the action (`reviewed` on PROMOTE_*), and the reducer's single `promote()` rebinds it to the new Active, so the analysis is fresh at once; with no reviewed plan it uses the inspected mirror or the draft's fresh plan, else re-solves, and never shows an optimizer's own plan. A pad map matching candidates (`layoutIdentity`, on the derived layout) promotes as them: every matching card leaves the list and Undo returns them. The replaced Active is auto-saved as a variant and an unrelated draft goes to Recovered drafts. The trace carry-over lives in one helper (`traceAfterPromote`, extended by S3.4). Tests: `test/ui/state/singlePromote.test.tsx` (7: every source, undo, staleness after rebinding), `LayoutOptionsPanel.test.tsx`, `layoutStateBar.test.tsx`, `test/e2e/one-promote.spec.ts`, c2, c7.
- [x] Freshness rules (T14) · *PR / verified by:* PR #109 (cd9779e). `state/analysisInputs.ts` decides staleness: only notes, mute/solo scope, tempo, placements, locks and finger preferences mark the analysis stale, in the reducers, the lane actions (`lanesReducer`'s `withSyncedStreams` and SYNC_STREAMS_FROM_LANES) and Undo/Redo (`restoreDocument`); renaming, recolouring, grouping and reordering Sounds leave it fresh, and the session copies of Sound names (candidates, analysisResult, inspectedAnalysis) follow an undone rename (the S1a.1 follow-up). A draft whose pads return to Active's is dropped (`draftOrNull`). During a re-solve the Analysis and Costs panels keep the previous numbers of the same subject, dimmed (`useShownAnalysis`, `data-updating`), and never flash empty; the state bar owns "Updating…". A candidate made for an earlier performance shows a "stale" chip and its run "performance changed since". Tests: `test/ui/state/freshnessRules.test.tsx` (19), `candidateRuns.test.ts` "stale candidates", `test/e2e/freshness.spec.ts`.
- [x] Unfinished, not failed (T25) · *PR / verified by:* PR #109 (cd9779e). Only placed material is scored: `scoringRequestFor` (layoutAnalysis.ts) filters the performance to the notes of placed Sounds for both the plan (auto-analysis and the cache's solve) and the LayoutScore, and the cache key follows; `hasPlacedNotes` gates auto-analysis. `VERDICT_TIERS` gains Unfinished (◐, dashed accent): a partly placed layout reads "Unfinished · 3 of 7 Sounds placed" with "Scoring covers the N notes you can play so far"; Infeasible is reserved for placed notes that can't be played. The unplaced Sounds are listed with drag handles (`panels/UnplacedSounds.tsx`; a click arms click-to-place). The timeline draws unplaced notes outlined (`data-placement="unplaced"`), never hidden (invariant 4), and a click on one selects the event at that time. The scope line reads "Analysing 4 of 7 Sounds · 2 muted · 1 not placed yet". Tests: `test/ui/analysis/placedOnlyScoring.test.ts` (4), `FeasibilityBadge.test.tsx` (Unfinished cases), `selectedEventVerdict.test.tsx`, `analysisScope.test.ts`, `test/e2e/unfinished.spec.ts`, c6 (a partly placed layout is Unfinished, never Feasible).
- [x] Explicit fill-in (T37) · *PR / verified by:* PR #109 (cd9779e). "Place remaining N Sounds" appears in the unplaced list and the Sounds panel header whenever Sounds are unplaced (`usePlaceRemaining`). Under Q4 it adds one candidate, "Remaining placed", as its own run (ADD_CANDIDATE_RUN), shown read-only at once; it writes nothing and records no undo step. The candidate keeps every placed Sound and lock exactly where they are (`withRemainingPlaced`, shared with Suggest's natural-hand-pose fill), and its plan and Score come from the cache. "Use as my draft" applies it in one click, one undo step (the question is skipped when the source keeps every Sound and lock of the draft). On an empty grid the Generate button reads "Generate layouts from scratch" and still only proposes. Tests: `test/ui/hooks/placeRemaining.test.tsx` (4), `unfinished.spec.ts` (second case).
- [x] Compare on the cache (T08 full) · *PR / verified by:* PR #109 (cd9779e, b7abf49). CompareModal is rebuilt on the per-layout cache: both sides always show that layout's own plan and Playability (`analyseLayoutCached`/`useLayoutAnalysis`), never a stub or an optimizer plan. Candidate letters are stable for the session (`state/candidateRuns.ts`: a letter is assigned when a candidate is installed and never renumbered after a delete, a Promote or a new run). PadGrid draws the layout's pads first, then the plan's fingers over them. Each side has Promote and Keep. `summarizeComparison` takes the sides' names (it said "Candidate B is 3% easier" for Candidate A). Tests: `test/ui/components/compareOnCache.test.tsx` (3), `test/e2e/compare-cache.spec.ts`, c7.
- [x] Keep candidates (T30) · *PR / verified by:* PR #109 (cd9779e). Keep is on every candidate row, in View all, on the state bar ("Keep as variant") and on each Compare side, all through `useLayoutActions().keep`: SAVE_AS_VARIANT with source candidate, named after the strategy ("Natural hand pose, shifted 1 row"), with an Undo toast; the button then reads "Kept". Runs are grouped ("Run 2 · Quick · just now", from the candidates' metadata, so generation is unchanged); each Generate or Place remaining adds a run, earlier runs fold under "Earlier runs (n)" (capped at 3) with "Clear older runs", and `state.candidates` stays one flat list, newest run first. The caption reads "Candidates are temporary · Save the ones you like as variants". Leaving warns while candidates are unkept (the "← Library" popover "4 candidates aren't kept · Stay / Leave anyway", and beforeunload); candidates are never persisted. Tests: `test/ui/state/candidateRuns.test.ts` (7), `test/e2e/keep-runs.spec.ts`, `readable-names.spec.ts`.
- [x] Learn More, part of the P3 item: the Unfinished verdict tier and placed-only scoring · *PR / verified by:* PR #109 (cd9779e). The Unfinished tier renders from `VERDICT_TIERS`; a placed-only scoring paragraph (`learn-placed-only`) quotes `verdictHeadline()` itself through `UNFINISHED_EXAMPLE`; the Verdicts, Sound Identity and App Flow text follow (Keep, temporary candidates, Promote with Undo). Tests: `LearnMoreModal.test.tsx` "explains placed-only scoring and the Unfinished tier…" and "says Promote acts at once…".

**Exit criteria**
- [x] **P3-3** Bar, card, variant and modal Promote of the same candidate produce an identical activeLayout and plan hash, as one undo step with one toast. · *PR / verified by:* PR #109 (8f4a981). `test/e2e/one-promote.spec.ts` at 1366 and 1600: the same candidate promoted from Compare, from its row, from the state bar while inspected, and from a variant row after Keep gives an identical Active hash, a plan bound to it (`status().planLayoutHash`) and identical fingering, each as one undo step with one toast and no native dialog; the toast's Undo restores Active and the candidate. Unit: `singlePromote.test.tsx`.
- [x] **P3-4** C7 on the cache: with a differing draft, Active's Playability in Compare equals Active's standalone analysis. A greedy candidate shows the same score before and after "Use as my draft". · *PR / verified by:* PR #109 (8f4a981, b7abf49). `test/e2e/compare-cache.spec.ts` (Greedy, Natural Pose) at 1366 and 1600: with a differing draft, Active's Score in Compare equals its standalone Analysis tile; the candidate reads the same Score on its row, while inspected, as the draft after "Use as my draft", and in Compare; every placed pad on both Compare grids shows its finger. The S3.3 C7 cache case ("with a differing draft, Active's score in Compare equals its standalone analysis", flipped early in S1b.3) passes on the cache path: c7 5/5 at both sizes.
- [x] **P3-5** A rename leaves analysisStale false. After a lock toggle, the test awaits a plan whose layoutHash matches the current layout with analysisStale false; there is no wall-clock threshold. Opening the Composer tab changes neither analysisStale nor updatedAt. · *PR / verified by:* PR #109 (8f4a981). `test/e2e/freshness.spec.ts` at 1366 and 1600: a rename leaves analysisStale false with the plan still bound to the layout; after a lock toggle it polls until `planLayoutHash` equals the locked layout's hash with nothing stale (no wall-clock threshold), and a MutationObserver saw the panel dimmed but never empty meanwhile; opening the Composer tab and going back leaves analysisStale false and updatedAt unchanged. Unit: `freshnessRules.test.tsx`.
- [x] **P3-6** A 3-of-7 layout reads "Unfinished · 3 of 7 placed", and never "Infeasible" unless a placed Sound is unplayable. Its unplaced notes stay visible in the timeline. · *PR / verified by:* PR #109 (8f4a981). `test/e2e/unfinished.spec.ts` at 1366 and 1600: 3 of 7 placed reads "Unfinished · 3 of 7 Sounds placed" (see Deviations for the word "Sounds") with a plan that has no unplayable note; it reads neither Infeasible nor Feasible with each of 6 events selected; every note of the 4 unplaced Sounds is drawn outlined in the timeline (transparent fill, solid border). Unit: `placedOnlyScoring.test.ts`, `FeasibilityBadge.test.tsx`.
- [x] **P3-10c** The Learn More sync test covers the Unfinished tier. · *PR / verified by:* PR #109 (cd9779e). `LearnMoreModal.test.tsx` renders the Unfinished tier from `VERDICT_TIERS`, the badge as it arises (a feasible verdict with Sounds unplaced) and the placed-only scoring text.

**Session checks**
- [x] Full suite before push. · *PR / verified by:* on b7abf49 (fast-forwarded onto the phase branch, same tree): `npm run typecheck` (the e2e specs also typecheck under a temporary tsconfig); `npm run test:run` 114 files, 1306 passed, 4 `runIf` skips, including `testMidi1Integration.test.ts` (0 unplayable events); `npm run build` + `npm run check:no-test-hook`; Playwright at chromium-1366 and -1600: the new `one-promote`, `freshness`, `unfinished`, `compare-cache` and `keep-runs` (12/12), c1–c9 (C9's S8.2 expected-fail case still fails as documented), `inspect`, `state-bar`, `one-yardstick`, `clean-names`, `library-trust`, `readable-names`, `overlays`, `input-table`, `measured-grid`, `readability`, `composer-data`, `distinct-sounds`, `hook` and `a11y-library` all passed.

#### S3.4 — Generation progress, Cancel and time budget; trace per candidate

- **Status:** In progress (branch `claude/amazing-mayer-n3xp33`, PR #109)
- **Prerequisites:** S3.3.
...
| P0-6 | pass | Nightly-only. Re-triggered on 09da4cc by this audit: [36072965133](https://github.com/TGALLOWAY1/PushFlow-Modified/actions/runs/36072965133) (workflow_dispatch, 2026-09-24) passed in 49 min 17 s; the deep-annealing job (`npm run test:nightly`) took 48 min 57 s and the Firefox job 3 min. Updated by the P1b audit (2026-09-25), which also re-triggered it on 0a4eee6 ([36093551155](https://github.com/TGALLOWAY1/PushFlow-Modified/actions/runs/36093551155); see P1b). Updated by the P2 audit (2026-09-25): 36093551155 passed in 50 min 42 s (deep annealing 50 min 37 s, Firefox 3 min 10 s), the scheduled run on 62fcaf9 ([36118724717](https://github.com/TGALLOWAY1/PushFlow-Modified/actions/runs/36118724717)) passed in 50 min 9 s, and the P2 audit's re-trigger on d651978 ([36172516953](https://github.com/TGALLOWAY1/PushFlow-Modified/actions/runs/36172516953)) passed in 49 min 55 s (deep annealing 49 min 51 s, reporting 49.6 min, and 48.8 min with the lock; Firefox 4 min 16 s). |
...
- Date: 2026-09-25
- Commit: d651978 (main after PR #107)
- Gates on this commit: `npm run typecheck` clean; `npm run test:run` 98 files, 1146 passed, 4 `runIf` skips (80 s); `npm run build` + `npm run check:no-test-hook` OK; `PW_CHROMIUM=/opt/pw-browsers/chromium npx playwright test` (chromium-1366, chromium-1600, and C1 at chromium-1920): 183 passed, 2 failed (6.8 min). The two failures are the Library screenshot comparisons (8,514 and 8,479 pixels, 1% of the image, glyph edges only), which differ from the CI baselines in a cloud container (S1a.1 follow-up) and pass in CI. The only `test.fail` markers left are C2's Inspect case (S3.2) and C9's timeline-insert case (S8.2); both still fail as documented, and no expected-fail case passed. TEST MIDI 1 (`testMidi1Integration.test.ts`): 34 passed, 4 `runIf` skips; 0 unplayable events for Greedy, Beam and Annealing Quick, with and without the [7,0] lock and with a muted Sound; the three seeded snapshots match.
- CI on the same commit: [run 36163296556](https://github.com/TGALLOWAY1/PushFlow-Modified/actions/runs/36163296556) (#107 merge, 2026-09-25, 3 min 32 s), every job green: typecheck, unit, build with "No window.__pf in dist/", and e2e ×2 shards.
- Nightly: no P2 criterion is nightly-only. But P2 changed engine text (greedy trace move descriptions, difficultyScoring's binding-constraint text, presetTransform's drop reasons) and added six e2e specs that had never run in Firefox, and no nightly had run since #107, so this audit re-triggered it on d651978: [36172516953](https://github.com/TGALLOWAY1/PushFlow-Modified/actions/runs/36172516953) (workflow_dispatch, 2026-09-25). Its Firefox job ran the whole e2e suite, P2's specs included, at firefox-1366 and passed (4 min 16 s). Its deep-annealing job passed too (49 min 51 s): every candidate has 0 unplayable events in strict mode, and the lock at [7,0] holds in every candidate (3 tests), with the durations reported as 49.6 min, and 48.8 min with the lock. See P0-6 for the earlier runs.
- Engine check (Solver Change Checklist): P2's engine diff is text only. `greedyOptimizer.ts` move descriptions and `presetTransform.ts` drop reasons read "Row 4 · Col 4" (`formatRowCol`), and `difficultyScoring.ts`'s binding-constraint text counts events and notes through groupIntoMoments. `OptimizerMove` and `AnnealingIterationSnapshot` fields, `stopReason`, the `isProcessing` resets (`useAutoAnalysis.ts` changed two disabled-reason strings only), MoveTracePanel, `annealingSolver.ts`, `optimizerInterface.ts` and `src/engine/solvers/` are untouched since 62fcaf9. S2.2a regenerated the three seeded snapshots for the new Sound names; this audit mapped each old "TEST MIDI 1 N" to the N-th letter (70 entries), after which the old file equals the new one byte for byte, so no pad, score or hand count changed for any method.
- Independent probes: besides re-running every spec, this audit drove the app with its own Playwright probes (a scratch config, not committed: one worker at 1366×768 and 1600×1000 on its own VITE_E2E server). Each checks a criterion another way than the session's spec where it could ("Probe" below). The final run passed 9 probes at both sizes (18 runs, 2.2 min). The earlier runs failed only on the probes' own mistakes (an assumed variant order, a slider `fill`, reads taken before the app had settled, and two text matchers), not on app defects.
- Screenshots (this audit, `docs/screenshots/audit-P2/`, 1366 and 1600): `01-workspace`, `02-variants-scored`, `03-library`, `04-compare`, `05-click-to-place`, `06-composer`. PWC below means the Playwright case passed at 1366 and 1600 in the full local run.

| Criterion | Result | Evidence |
|---|---|---|
| P2-1 | pass | `measured-grid.spec.ts` "all 64 pads…" and "every transport control…" PWC (pads 34 px at 1366, 51 px at 1600). Probe: with TEST MIDI 1 suggested, no pad, hand-zone label or the state-bar slot is cut by the viewport or by a clipping ancestor (checked per axis), and all eight transport items (Play, Return, the position, Speed, Loop, Metronome, Hits and "⋯") take a click at their centre. The same holds with 16 Sounds (the drawer grows to 280 px at 1366 and 372 px at 1600, and the timeline shows all 16 lanes) and with the Composer tab shown (pads stay 34 and 51 px). Screenshot 01. |
| P2-2 | pass | `measured-grid.spec.ts` "after importing a 4-bar clip at 120 BPM…" PWC. Probe: the ruler equals its container after the import (484 px at 1366, 718 px at 1600), and again after narrowing the window by 180 px, widening it by 120 px, restoring it, and a Composer and back tab switch. A reading taken within about half a second of a resize can differ while the side panels give way; settled readings are equal at every width sampled from 1100 to 1366 px. |
| P2-3 | pass | `distinct-sounds.spec.ts` both P2-3 cases PWC; `distinctSounds.test.ts` "opening a project saved before S2.2a keeps every custom colour and name" (custom colours unchanged). Probe: the 7 imported colours (#E69F00 … #CC79A7) are pairwise at least 21.72 apart in CIEDE2000 (closest: #E69F00 and #F0E442). chroma's `deltaE` and an independent CIEDE2000 implementation, which reproduces Sharma et al.'s reference values, agree to 1e-10. (chroma round-trips Lab through RGB, so it differs only for out-of-gamut Lab pairs, never for hex colours.) The pad labels "A" … "G" all render without CSS truncation, and the page's visible text has no note name. Screenshot 01. |
| P2-4 | pass | `readability.spec.ts` "no grid or timeline text is under 11 px" PWC (default labels, every label on, an event selected). Probe: none either while playing, with onion skin on, with a locked pad, at the zoom slider's maximum and minimum, in the timeline's "⋯" popover, with 16 Sounds, or in the Compare dialog's grids. |
| P2-5a | pass | `library-trust.spec.ts` "cards show real data…" PWC; `libraryFacts.test.ts`. Probe: after a second project, made with the Library's Import MIDI, takes the hero, the TEST MIDI 1 card reads "8 bars · 32 events · 7 Sounds · 120 BPM" and "Created Sep 25 · Opened just now · 2 variants", and the hero "4 bars · 32 events · 3 Sounds · 120 BPM". The stored `createdAt` is unchanged after editing, promoting and reopening. Screenshot 03. |
| P2-5b | pass | The same spec PWC. Probe: the draft-only card draws its 7 pads, badged "Draft, not promoted"; after Promote it draws the same 7 pads, badged "Active layout". Screenshot 03. |
| P2-5c | pass | `library-trust.spec.ts` "Import MIDI creates "TEST MIDI 1"…" PWC; `libraryActions.test.ts`. Probe: the Library's Import MIDI with `four-bars-120.mid` creates "four-bars-120" with 3 Sounds, no pad in the Active Layout, no draft and bottomLeftNote 36; the demo project has nothing placed until it is placed by hand (the P2-11 probe). |
| P2-6 | pass | `library-trust.spec.ts` "distinct names in the same minute…" PWC; `variantCard.test.tsx`. Probe: two saves in one minute are "Default – 25 Sep 18:46" and "… (2)". The second, renamed "Wide hands" (found by id, since the list shows the newest first), keeps its name after a save and reload. A MutationObserver installed at document start saw both cards read "Scoring..." after the reload and then "Score 79% · 0 hard · 0 unplayable", with nothing in between; before the reload the cards score at once from the analysis cache. Screenshot 02. |
| P2-7 | pass | `library-trust.spec.ts` "Delete, then Undo within 10 s…" PWC; `libraryActions.test.ts`. Probe: after Delete the card is gone and the stored record is removed within 121 ms, the Undo toast is still offered 8.6 s later, and Undo restores the stored record exactly (same id, Active Layout and 2 variants). |
| P2-8 | pass | `readableReferences.test.ts` "…the C7 case"; `readable-names.spec.ts` PWC. Probe: after Promote and a Beam Generate, Compare reads "7 Sounds moved (13 pads changed)" for candidates #1 and #2 and "7 Sounds moved (14 pads changed)" for Active against #1. Both equal the audit's own count of unique Sound ids moved and unique pad keys changed between the two layouts. Screenshot 04. |
| P2-9 | pass | `factorMetaGrep.test.ts` (4 cases); `LearnMoreModal.test.tsx` "renders its factor list from FACTOR_META, in order, with the registry's colours". Probe: Learn More shows Movement, Grip, Alternation and Hand balance, and the Constraints and "Keyboard & mouse" tabs. |
| P2-10 | pass | `inputTable.test.tsx`: one test for each of the 20 rows plus "every row has a test, and every test a row" (29 tests); `input-table.spec.ts` P2-10 cases PWC. Probe: Space plays, then stops, with the Play button itself focused (no double toggle from the button's own key handling), with Save variant focused (its name field never opens) and with Undo focused (undo history unchanged), and focus stays put. ArrowDown on the focused Optimizer method select changes it (Greedy to Beam) and leaves the selected event where it was. |
| P2-11 | pass | `input-table.spec.ts` three P2-11 cases PWC. Probe: on the demo project, one Sound click and seven pad clicks place all 7 Sounds (auto-advance), 7 undo steps, nothing left armed, and the analysis then reads Feasible. Screenshot 05 (G armed, its dashed preview under the pointer). |
| P2-12 | pass | `readable-names.spec.ts` PWC. Probe: no "lane_" and no current Sound or voice id in the workspace's visible text or in any title, aria-label, placeholder or alt with 4 of 7 Sounds placed (the verdict reads "Infeasible · 14 of 48 notes can't be played"), nor in the Compare dialog's text or attributes, for two candidates and for Active against #1. The spec covers the Library. |
| P2-13 | pass | `input-table.spec.ts` P2-13 cases PWC; `gridSizing.test.ts`. Probe: bar 8's header, its line and its first cell line up to 0 px at 8 bars 1/4 and at 16 bars 1/4, 1/2 and 1/8, also after narrowing the window by 160 px and restoring it. Cells are 16–21 px and edge to edge, and the first note enables Save Preset without moving the toolbar or the grid. Screenshot 06. |

**CLAUDE.md rules, checked on d651978**

- Product Invariants. (1) Desktop-only: P2 adds no touch handlers. The one responsive class in P2's diff, `hidden lg:block` in `Homepage/ProjectHero.tsx`, moved from the deleted `ContinuePracticingHero.tsx` (pre-roadmap, bb0360f), and `ProjectLibraryPage.tsx`'s grid classes also predate the roadmap. (2) Learn More sync: P2's factor labels, verdict wording, App Flow and input table landed in `LearnMoreModal.tsx` (Cost Factors from FACTOR_META, the "Keyboard & mouse" tab); see the follow-up on the lock rules' list of gestures. (3) The Composer is still the `drawer-tab-composer` tab, and a collapsed drawer keeps the tab bar (`measured-grid.spec.ts`). (4) Timeline completeness: probe, with 4 of 7 placed all 7 lanes show, and the 3 unplaced Sounds' 14 notes are red Unplayable pills, none hidden; with 16 Sounds, 16 lanes. (5) `midiImport.ts` and `midiImportPlan.ts` (shared by the Library and the editor) never change bottomLeftNote (probe: 36 after a Library import); default names come from the file or track (`soundNaming.ts`), and pitch only tells Sounds apart. (6) Finger sync: `FingerAssignmentInput` is unchanged by P2 and still writes only voiceConstraints. (7) No automatic layout: the Library import, a .mid dropped on the grid and the demo place nothing (P2-5c); click-to-place is one named action and one undo step per pad (P2-11), and the auto-advance only arms the next Sound. (8) The Composer shows the project tempo read-only ("120 BPM", titled "The Composer plays at the project tempo"); there is no BPM control.
- Core Functionality Preservation and Do-Not-Regress. See the engine check above. Multi-candidate generation, diversity filtering and Compare are intact (P2-8's probe compares two candidates, and Active against one). Both trace shapes and `stopReason` are unchanged, and MoveTracePanel is still mounted in PerformanceWorkspace. Toolbar Generate still leaves the trace empty, the known S0.2 follow-up that S3.4 owns.
- UI Non-Regression, probed at both sizes. The timeline fills its width (P2-2). Pills read hand and finger ("L2", "L3", "R2", "R3"), never a bare digit; the unlabelled pills are the unplaced Sounds' Unplayable notes. A click on one note of the 3-note chord at 1.1.1 rings exactly those 3 pills. FingerAssignmentInput shows the solver's suggestions at opacity 0.5, and no Sound row shows "(L2)"-style text. "Show Finger Assignment" shows the solver's fingering on all 4 placed pads, hides it when off, and shows it again when on. Library cards show real data (P2-5a). With a group, the other Sounds read "Ungrouped (n)", never "On Grid". Ctrl+G groups two selected Sounds, and selecting them again and pressing Ctrl+G ungroups them.
- One-line fixes made by this audit: none needed.
...
- **2026-09-25 · S2.1 · Review on #107: small windows and wide panels.** Codex found two gaps in S2.1. (1) In a centre column shorter than the grid's minimum plus an open drawer (about 522 px), the drawer kept its 120 px and the grid was clipped; the grid's minimum now wins and the drawer shrinks toward its tab bar (`drawerSizing.ts`: `maxDrawerHeight`, `minDrawerHeight`; the splitter's range follows). (2) Widening a side panel, or any window narrower than about 1300 px with the default panels, squeezed the centre below the transport cluster plus its "⋯" button (`TIMELINE_TOOLBAR_MIN_WIDTH`), which clipped Metronome, Hits and "⋯"; the side panels now keep the widths the viewer dragged to but are shown only as wide as leaves the centre `CENTER_MIN_WIDTH` (`workspace/panelSizing.ts`, proportionally, never below their minimums), drags stop there, and a window narrower than even the minimum panels scrolls the timeline toolbar instead of clipping it. Nothing changes at 1366 and 1600 with the default panels. Tests: `gridSizing.test.ts` (short centre, panel fitting, drag limit) and `measured-grid.spec.ts` "short windows, narrow windows and wide panels" (1366×540, both panels dragged wide, 1200×768), which fail on the previous code with the zone labels, Metronome, Hits and "⋯" clipped.
...
- **2026-09-25 · S2.3 · "Last opened" is stored with the project (schema 4).** T52 asks to track last-opened for the hero and cards; it is a project field like createdAt, written by a metadata-only save when the editor opens a project, so it travels with an exported file and survives other browsers' storage rules. A duplicate or an imported file starts as never opened ("Not opened yet") and sorts by its creation.
- **2026-09-25 · S2.3 · Opening a project no longer re-saves it.** Not listed in S2.3, but it is T52's cause of "just looking at a song moves it into the hero": the editor's mount-time Sounds sync bumped updatedAt even when it rebuilt identical Sounds. An equal rebuild is now a no-op (which also keeps a fresh analysis), and the unmount flush skips a state already being saved.
- **2026-09-25 · S2.3 · The "nothing placed yet" step lives in the state bar, not over the grid.** T44 puts the staged empty state in the grid's centre. The first stage (no Sounds) is a card there; the second (Sounds, none placed) is one line with the Suggest button in the state-bar slot above the grid, because a card over the pads would cover the drop targets for Sounds and presets. "Click a Sound then a pad" joins that line with S2.4's click-to-place.
- **2026-09-25 · S2.3 · One Suggest button, one import CTA, one New project.** The Analysis panel's "Suggest a starting layout", the timeline's "Import MIDI Files" and the sidebar's "New Performance" duplicated the new staged empty state and header; they are gone (their places keep a line of guidance), so each action has one home and one accessible name.
- **2026-09-25 · S2.3 · The sidebar keeps "View presets" and the developer tools.** T52 notes that "View presets" with no project creates one; it still does (opening the Presets panel needs a project). The Constraint validator and Temporal evaluator links stay: the roadmap keeps the debug routes.

- **2026-09-25 · S2.4 · The "?" sheet has no top-bar button yet.** The north star puts a "?" button in the top bar. At 1366×768 with a draft the bar has no room left (a 28 px button cut the project name to "Untitled ..."), so the sheet opens on "?", from the gear menu ("Keyboard and mouse ?") and as Learn More's "Keyboard & mouse" tab. The button can join the top bar when P3 moves freshness into the state bar or P7 consolidates the bar (see Follow-ups).
- **2026-09-25 · S2.4 · A pad click selects the pad, not the Sound's first hit.** Per the P2 input table: a click selects the pad and its Sound, and with an event selected it keeps the event. Before, it selected the pad's first event (T28's complaint), and an empty pad cleared the event. Delete now acts on the selected pad, as the table says; the pad inspector, "Show its hits" and Delete's toast wording stay with P4 (T28).
- **2026-09-25 · S2.4 · ↑/↓/j/k need focus in the Events list.** They worked whenever the Events tab was mounted. T61 scopes them to a focused Events list, so they are the table's `events-list-keys` row, bound to the list (click an event, then step); ←/→ step events from anywhere.
- **2026-09-25 · S2.4 · Alt-click does nothing until P4.** The table reserves Alt-click for audition. A reserved meaning does nothing rather than act as a plain click, so no habit forms around it.
- **2026-09-25 · S2.4 · "Next unplaced" is always on, and arming is idempotent.** The roadmap makes the auto-advance optional; it is on, and Escape or "Stop placing" ends it. A click on the armed Sound keeps it armed rather than toggling it off, which would disarm the Sound the auto-advance had just armed. Moving a Sound that is already placed ends placing.
- **2026-09-25 · S2.4 · Space in the Composer plays the pattern.** "Space plays and stops everywhere", but with the Composer tab shown there are two transports and only the Composer's is on screen, so Space drives that one (the Composer binds Space at a higher priority while its tab is shown). S8.2 puts the Composer on the shared transport.
- **2026-09-25 · S2.4 · The Composer gets the drawer's 40% height.** S2.1 fits the drawer to the timeline's content. With the Composer tab shown it takes its 40% default instead (or the viewer's dragged height), because following the Sound count moved the Composer up a lane with each new lane's first note. The drawer changes height when switching tabs unless the viewer has set one.
- **2026-09-25 · S2.4 · The Composer's title and counts moved.** "Pattern Composer / Changes sync directly into the shared performance timeline." left the toolbar (the tab names it, and the sentence is now a tooltip), and "N lanes · N events · live sync" became "Lanes · N notes · live sync" in the lane column's header, so the toolbar fits one row at 1366. The Composer's cells are notes (Q7).

- **2026-09-25 · P3 · One PR for the phase, built by parallel agents.** The user asked for all of Phase 3 with subagents taking separate parts. The session's branch is fixed (`claude/amazing-mayer-n3xp33`), so S3.1–S3.4 land as ordered commit groups in PR #109 (as P2 did in #107). Packages with no dependency on each other were built at the same time in separate git worktrees (S3.1's primitive kit with S3.2's clean-names slice; S3.1's yardstick; S3.4's engine work) and merged in prerequisite order: all of S3.1's code before S3.1 was marked Done, S3.2's clean names only after that, and S3.4's engine work only once S3.3 is Done. Each session moves to In progress only when its prerequisites are Done (Codex review on #109). Approved by: the user asked for it; flagged in the PR.
- **2026-09-25 · S3.1 · Playability's counts are the canonical evaluator's.** Q5 puts Hard and Unplayable event counts beside the headline. Rows that show counts next to the Score (variant and Active rows) take them from the same LayoutScore, i.e. canonical hard events (fallback grips) and canonical unplayable events. The Analysis panel's Hard/Unplay tiles, the verdict line, the Events list and chart, and the candidate card's labels still count by the plan's cost bands until S6.1 unifies them (see Follow-ups), so one layout can read Score 88% with Hard 4 beside it: the plan's bands call 4 events hard while no grip breaks a rule.
- **2026-09-25 · S3.1 · "Scoring…" with a real ellipsis.** P2-6's text says "Scoring..."; the rows now read "Scoring…", like "Analysing Active…".
- **2026-09-25 · S3.1 · Compare's Score is an integer percent.** It showed the plan score to one decimal ("85.3"); it now reads "91%", like every other surface.
- **2026-09-25 · S3.1 · The Active row gets a score.** The Active card had no score; it now shows the same "Score N% · N hard · N unplayable" line as variant rows, from the same cache.
- **2026-09-25 · S3.1 · Compare's Active side is the cache's plan.** It read `state.analysisResult`. After a candidate Promote the Analysis panel still shows the promoted candidate's own plan (until S3.3's Promote rebinds the cache's plan), so plan-based tiles there can differ from Compare's Active side; the Score is the same.
- **2026-09-25 · S3.1 · LayoutScore also carries the cost per event.** It is the score's ergonomic term, kept for P6's share-of-burden presentation.
- **2026-09-25 · S3.1 · The focus ring uses the focus token.** `.focus-ring` (unused until the kit) drew in `--accent-primary`, which falls under 3:1 on hovered surfaces; it now draws in `--border-focus`.
- **2026-09-25 · S3.1 · A focused checkbox or tab row keeps its own keys.** The S2.4 table had Space play and ←/→ step events everywhere but text fields. A focused checkbox now takes Space (ticks it) and a tab row takes ←/→/Home/End (moves between tabs); only those keys, and Space on any button, tab included, still plays. The sheet's rows say so.
- **2026-09-25 · S3.2 · Clean names were built before the rest of S3.2.** The T32 slice (provenance, labels, the schema 5 migration) depends on nothing in S3.1, so it was built alongside S3.1 and merged right after S3.1's code (adc804c).
- **2026-09-25 · S3.2 · Replaced Actives are re-dated, not only stripped.** Stripping "(replaced 9/24/2026)" alone would leave two replaced Actives both called "Default", so the migration gives each the " – 24 Sep 14:02" stamp new ones get (from its savedAt), and numbers a variant name it makes taken (T29: no two variants share a name). "(draft)" alone infers no provenance: a manual edit and an applied candidate both wrote it.
- **2026-09-25 · S3.2 · Provenance is also set by Load draft and by the candidate and variant Promotes.** Otherwise a promoted replaced-Active variant would still claim to be a replaced Active.
- **2026-09-25 · S3.2 · Only a draft's label names its role.** Active, variant and candidate labels are the bare base name by default ("Active: …" is `withRole`), because every surface today shows the role in a badge beside it; the summary reads "Draft of Default" next to its DRAFT badge until the state bar's role chip replaces the badge.
- **2026-09-25 · S3.2 · Auto-inspect lives in SET_CANDIDATES.** Candidate A is shown by the reducer that installs a run's candidates, not by `generateFull`, so every path that installs candidates does it and the generation code is untouched. Any later action that installs a run's candidates must inspect A the same way.
- **2026-09-25 · S3.2 · The inspected plan is mirrored into session state.** `inspectedAnalysis` holds a read-only layout's own plan from the cache so the pure selectors (timeline, Events list, SELECT_EVENT's playhead, arrow keys) can read it; it is checked against the layout's hash before use and is never in undo or saved.
- **2026-09-25 · S3.2 · Recovered drafts can be inspected too.** The roadmap names active, working, candidate and variant; a recovered draft is shown read-only the same way (kind 'recovered'), with Use as my draft and Back.
- **2026-09-25 · S3.2 · A trace replay is read-only.** Stepping through the trace shows a historical layout; edits made then used to go to the draft unseen. They are refused with "Exit the trace replay to edit".
- **2026-09-25 · S3.2 · A Sound's finger preference still applies while inspecting.** SET_VOICE_CONSTRAINT is a Sound-level preference (invariant 6), so it runs and the inspection stays; the pad-level SET_FINGER_CONSTRAINT is refused like every layout edit.
- **2026-09-25 · S3.2 · Replace still keeps the old draft.** "Replace (undoable)" in Use as my draft also puts the replaced draft in Recovered drafts (the S1a.2 safety net), and its toast says so.
- **2026-09-25 · S3.2 · Undo and Redo that change layouts end the inspection,** so the grid shows what they restored.
- **2026-09-25 · S3.2 · The state bar is 36 px, not about 40.** At 40 px the pads at 1366×768 shrink from 34 to 33 px and an occupied pad's invisible × covers its centre, so a centre click removed the Sound (input-table failed twice). S2.1's 36 px slot holds the two-line role chip (30 px), so it stays.
- **2026-09-25 · S3.2 · The bar's Promote acts at once, with an Undo toast.** A row's Promote keeps its "Confirm?" until S3.3 makes one Promote; the draft's Promote, which had no feedback, now shows the toast.
- **2026-09-25 · S3.2 · Smaller choices.** With no draft, Back reads "Back to Active"; the one-time note sits inside the bar, not over the pads; Compare's sides read "Candidate A · name"; a candidate's name is its strategy label (its layout carries the user's base name).
- **2026-09-26 · S3.3 · One agent for the whole session.** The plan split S3.3 between two parallel agents; they would have shared the Analysis summary, the reducer, the Layouts list and Learn More, so one agent built it all.
- **2026-09-26 · S3.3 · "3 of 7 Sounds placed".** P3-6's text says "Unfinished · 3 of 7 placed"; the verdict reads "Unfinished · 3 of 7 Sounds placed", as the session prompt's example ("5 of 7 Sounds placed") does.
- **2026-09-26 · S3.3 · A partly placed Degraded layout also reads Unfinished.** Unfinished wins over Degraded while Sounds are unplaced (the summary keeps "· N hard events"); Infeasible still wins over both when placed notes can't be played.
- **2026-09-26 · S3.3 · Use as my draft skips the question when nothing would be lost.** When the source keeps every Sound and lock of the draft (Place remaining's candidate always does), it applies in one click (Q4's one-click apply); the reducer still keeps the old draft in Recovered drafts.
- **2026-09-26 · S3.3 · A Promote with no cached plan re-solves.** It never rebinds an optimizer's own plan to the new Active, which settles S3.1's "Compare's Active side is the cache's plan" deviation: after a Promote the Analysis panel shows the cache's plan too.
- **2026-09-26 · S3.3 · Run titles come from the candidates.** Beam and annealing runs read "Quick" or "Thorough" (their optimizationMode) and greedy runs "Greedy · <family>", so generation's dispatches are unchanged.
- **2026-09-26 · S3.3 · Only an identical draft is dropped.** An emptied draft over a non-empty Active is kept: removing every Sound is a real edit.
...
- **2026-09-24 · S1a.2 · Recovered drafts aren't in the Library card or the reopened-draft banner.** A project whose only copy of some work is a recovered draft looks the same in the Library as any other. Belongs to: S2.3 (Library card) and S5.4 (reopened-draft banner). **Partly done in S2.3:** a card whose project opens on an unpromoted draft draws the draft and badges it "Draft, not promoted" (recovered drafts, kept separately, aren't counted on the card); the banner stays with S5.4.
...
- **2026-09-24 · S1a.4 · The Delete key on a locked pad does nothing silently.** The reducer refuses `REMOVE_VOICE_FROM_PAD` for a locked pad; the × button and the menu item say why, the keyboard path does not. A toast ("Locked · Unlock to remove") belongs with P3's lifecycle confirmations. Belongs to: S3.3 (T31). **Resolved in S2.4** (checked by the P2 audit): Delete on a selected locked pad shows "TEST MIDI 1 A is locked · Unlock it to remove it" and removes nothing.
...
- **2026-09-25 · S1b.1 · Counts still mix notes and events.** With four of seven Sounds placed, the Costs panel shows "EVENTS 32" beside "14 unplayable events" and "14 unplayable of 48 events" (note counts), and "Score 91%" beside 14 unplayable. Out of scope here. Belongs to: S2.2b (T23 counts) and S6.1 (one headline score, T21). **Counts done in S2.2b** (checked by the P2 audit: with 4 of 7 placed, the Costs tab reads "EVENTS 32 · HARD 2 · UNPLAY 11" beside "14 of 48 notes can't be played" and "11 unplayable events"); the single headline score stays with S6.1.
...
- **2026-09-25 · S1b.2 · The M key during a preset drag is still wired.** PerformanceWorkspace listens for M while a preset drag is active, though browsers don't deliver it during a native drag; the visible Mirror toggle replaces it. Remove it with the input table. Belongs to: S2.4. **Resolved in S2.4:** the listener is gone.
- **2026-09-25 · S1b.3 · Compare labels still use list positions.** "#1 pose0-offset-0" renumbers after a delete, and the Layout differences line lists raw `lane_` ids. Belongs to: S2.2b (readable references) and S3.3 (stable candidate labels). **Partly done in S2.2b:** Compare names Sounds and strategies ("#1 Natural hand pose"), with no `lane_` ids (P2-12); the list-position numbers stay with S3.3.
- **2026-09-25 · S1b.3 · Variants don't use the cache yet.** `getAnalysisForLayout` serves Compare and auto-analysis; S2.3's variant scores and P3's inspection are its next users. **Done in S2.3** for variant cards, which score through `useLayoutAnalysis` (P2-6); P3's inspection is next.
- **2026-09-25 · S1b.4 · No drag-off-the-grid removal.** The roadmap lists "dragging off" as a removal gesture with an Undo toast; dragging a pad off the grid does nothing today. Belongs to: S2.4 (input table) or S3.2. S2.4 didn't take it: the P2 table doesn't list it, and a drag that ends off the grid can't be told from a cancelled one without a page-wide drop target, so it stays with S3.2.
...
- **2026-09-25 · P1b audit · C1 doesn't bound the pad menu's width.** The portalled menu stretched to the viewport edge (630–884 px) while all C1 cases passed, because they check position, clamping and clickability. Fixed in the audit (`flex flex-col`); C1 should also assert a sensible width (for example under 320 px) so it can't come back. Belongs to: S2.4 (input table, which reworks the pad menu) or S3.1 (primitive kit). **Resolved in S2.4:** C1 requires under 320 px on all 64 pads at 1366, 1600 and 1920.
...
- **2026-09-25 · S2.3 · New projects are saved before their first edit.** "New project" (and "View presets" with no project) stores an empty "Untitled project" at once, so an abandoned one lingers and can be the "Last opened" hero. T52 recommends saving only after the first meaningful action or discarding empty projects on leave. Belongs to: S5.4 (Library complete).
- **2026-09-25 · S2.3 · Reopened drafts aren't announced.** Opening a project whose work is an unpromoted draft restores it silently; the card now badges it "Draft, not promoted", but the editor has no "Draft from 23 Sep restored · Keep editing / Discard" banner (T52). Belongs to: S5.4 (the S1a.2 follow-up already points there).
- **2026-09-25 · S2.3 · No drop overlay outside the empty grid.** A .mid dropped on the empty grid imports; elsewhere in the workspace or on the Library the browser still opens the file (T51's window-level "Drop MIDI to import"). Belongs to: S5.2 (import review).
- **2026-09-25 · S2.3 · Import-file collisions don't ask.** A project file whose id exists is kept as a numbered copy ("Name (2)"), never overwriting; T53's "Replace / Keep both" question is not asked. Belongs to: S5.4.

- **2026-09-25 · S2.4 · The top bar is full at 1366×768.** With a draft and the "Analysis outdated" badge the project name truncates to nothing (the badge takes about 150 px; also on main). S3.2's state bar carries freshness, so the badge can leave the top bar, and S7.1 consolidates it; the "?" button waits for that room (see Deviations). Belongs to: S3.2, S7.1.
- **2026-09-25 · S2.4 · Sounds can't be armed from the keyboard.** Sound rows are divs (T63), so click-to-place needs a mouse; pads are focusable only by script. Focusable Sound rows come with S3.1's primitives, and the roving-tabindex grid (Enter places the armed Sound) with S8.3. Belongs to: S3.1, S8.3.
- **2026-09-25 · S2.4 · Space doesn't activate a focused button.** Per the table Space plays everywhere but text fields, so a keyboard user presses a focused button (a toast's Undo, a tab) with Enter. The sheet says so under Space; revisit if users stumble. Belongs to: S3.1 (accessibility review).

- **2026-09-25 · P2 audit · Learn More's lock rules don't list S2.4's gestures.** The Placement Locks entry (`LearnMoreModal.tsx`, the `placementLock` description) says drags, drops, Remove from pad and presets respect a lock. Since S2.4, moving a locked Sound by click-to-place and removing it with Delete are refused too, each with its reason ("A is locked to Row 1 · Col 1 · Unlock it to move it"; "TEST MIDI 1 A is locked · Unlock it to remove it"). The text is incomplete rather than wrong ("Manual edits enforce locks too"). Add both gestures, and extend `LearnMoreModal.test.tsx`'s Constraints case. Belongs to: S3.3 (T31 lifecycle confirmations), or any earlier session that edits the Constraints tab.
- **2026-09-25 · P2 audit · The Material Symbols stylesheet is now unused.** Since S2.3 no element uses `.material-symbols-outlined`, but `index.html` line 11 still links fonts.googleapis.com for Material Symbols Outlined, so every page load makes a remote request (blocked in e2e by `test/e2e/fixtures.ts`), and `src/index.css` keeps the class's rule. Repro: open the Library with the network panel open; one request goes to `fonts.googleapis.com/css2?family=Material+Symbols+Outlined`, and `document.querySelectorAll('.material-symbols-outlined').length` is 0. Remove the link and the rule, and update the comment in `test/e2e/fixtures.ts` that expects it to go "in P5". Belongs to: S7.2 (tokens and primitives), or any session that edits index.html.
- **2026-09-25 · P2 audit · Ungrouping leaves an empty group header.** Repro: Ctrl-click two Sounds, press Ctrl+G, Ctrl-click the same two again, press Ctrl+G. Both Sounds are ungrouped, but an empty "Group 1" header with a count of 0 stays in the Sounds panel until it is deleted from its header. This predates P2 (the handler moved into the input table unchanged in S2.4); the Cmd/Ctrl+G toggle itself works. Belongs to: S5.1 (a real Sounds panel).

- **2026-09-25 · S3.1 · Counts beside the Score are still plan-based on most surfaces.** The Analysis panel's Hard and Unplay tiles, the verdict line, the Events list and chart, Compare's Hard/Unplayable/Events tiles, and the candidate card's Easy/Moderate label, "N unplayable" note and "Top:" driver count by the plan's cost bands, while the Score's inputs are canonical (e.g. 4 hard in the plan, 0 canonical). Belongs to: S6.1 (one headline, with its Hard and Unplayable counts).
- **2026-09-25 · S3.1 · Two numbers called "Playability".** Compare's tradeoff bar "Playability" (e.g. 78 / 83) is not the headline Playability (79 / 91). Rename it or derive it from the LayoutScore before the headline is relabelled. Belongs to: S6.1/S6.2.
- **2026-09-25 · S3.1 · One worker queue.** The scoring worker runs requests first in, first out, so after a large Generate the draft's analysis can wait behind the candidate rows (about 50 ms each on TEST MIDI 1, longer on long pieces). Belongs to: S3.4 (generation) or P7 (performance).
- **2026-09-25 · S3.1 · The cache's size is counted in entries.** 64 cached plans could reach tens of MB for performances with thousands of notes; a size-aware cap would bound it. Belongs to: P7 (performance) or S5.4.
- **2026-09-25 · S3.1 · The optimizer debug page shows the solver's own score.** `OptimizerDebugPage` (a developer route) still prints `result.score`. Belongs to: any session touching the developer routes; not a product surface.
- **2026-09-25 · S3.1 · The Checkbox's Space toggle has no browser test yet.** It relies on the native checkbox (the component test simulates the key sequence). The first surface that mounts a Checkbox should add an e2e Space check. Belongs to: the first P3+ surface using it (S3.3's Keep/compare boxes or P7).
- **2026-09-25 · S3.1 · A draft equal to Active still shows the Working/Test role (S1a.4 follow-up).** Not part of either S3.1 package. Belongs to: S3.2 (the state bar shows an identical draft as Active) and S3.3 (freshness: empty or identical drafts are dropped).
- **2026-09-25 · S3.2 · Engine candidates carry the user's layout name.** Candidate layouts inherit the base layout's name and provenance; the reducers set provenance on apply, keep and promote. The Lifecycle section (S3.4) should explain "Draft of …" labels and the dated names of auto-saved replaced Actives.
- **2026-09-25 · S3.2 · readable-names' greedy Generate is load-sensitive.** Its "verdict, Compare and Library text…" case timed out once inside greedy Generate at a load average of about 15 and passed 2/2 alone (1.6 of its 2 minutes). Belongs to: S3.4 (progress, Cancel and time budget).
- **2026-09-25 · S3.2 · Earlier follow-ups this session settled.** S1a.4's "a draft equal to Active still shows the Working/Test role" is done for display (it shows as Active; S3.3's freshness rules drop identical drafts from state). S2.1's "the state-bar slot holds the transition preview for now" is done (the preview is in the selected-event card; S4.2's docked inspector remains its home). S2.4's "the top bar is full at 1366×768" is partly done: the "Analysis outdated" badge and the Promote, Save variant and Discard buttons left the top bar, so the "?" button has room when S7.1 consolidates it.
- **2026-09-25 · S3.2 · No drag-off-the-grid removal (S1b.4 follow-up).** Not taken: it is not an S3.2 deliverable and needs a page-wide drop target to tell a drop off the grid from a cancelled drag. Belongs to: S5.1 (drag feedback, T46).
- **2026-09-25 · S3.2 · Rename and recolour still rewrite session copies (S1a.1 follow-up).** Candidate cards can show an undone name until the next Generate. Belongs to: S3.3 (freshness).
- **2026-09-25 · S3.2 · A pad's invisible × can cover its centre on small pads.** At 33 px or less (short windows), a centre click on an occupied pad hits the hover-only × and removes the Sound (with its Undo toast). Give the × a fixed 14 px size or an offset. Belongs to: S7.1 (or S7.2's target sweep).
- **2026-09-25 · S3.2 · Candidate letters follow list position.** `candidateLetter` letters by position, so a delete or a new run relabels them. Belongs to: S3.3 (stable letters).
- **2026-09-25 · S3.2 · At 1366 the candidate bar truncates its note and diff.** The full text is in the title tooltip. Belongs to: S7.1.
- **2026-09-25 · S3.2 · The editor chunk is over Vite's 500 kB warning.** `ProjectEditorPage` is 506.75 kB after S3.2 (488.95 kB before). A warning, not a failure; code-split the panels or the engine. Belongs to: S7.x.
- **2026-09-26 · S3.3 · Earlier follow-ups this session settled.** S1a.1/S3.2's rename and recolour session copies (they follow an undone rename); S3.2's and S1b.3's list-position candidate labels (stable session letters, in Compare too); S1a.4/S3.1/S3.2's identical draft left in state (dropped). S1a.4's "Delete key on a locked pad" was already resolved in S2.4.
- **2026-09-26 · S3.3 · Compare's tradeoff bars and the rows' Easy/Moderate label still come from the optimizer's difficulty analysis.** They sit next to the Score (the Playability yardstick) and can disagree with it. Belongs to: S6.1 (with the S3.1 follow-up on the two "Playability" numbers).
- **2026-09-26 · S3.3 · Recovered drafts collect the partial drafts that fill-in replaces.** Applying "Remaining placed" keeps the smaller draft in Recovered drafts although the new draft contains it. Consider not keeping a draft the incoming layout fully extends. Belongs to: S5.4 or a P3 audit one-liner.
- **2026-09-26 · S3.3 · The P2 audit's lock-rule gestures are still missing from Learn More.** The Constraints tab's lock text doesn't list the click-to-place and Delete refusals (P2 audit follow-up). Belongs to: S3.4's Learn More work or the P3 audit.
